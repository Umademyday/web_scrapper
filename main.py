...
if __name__ == '__main__':
    ...
    if filtering_type == 1:
        news_items = [x for x in news_items if x.words_count() > WORDS_NUM_FILTER1]
        news_items = sorted(news_items, key=lambda x: x.comments)

    elif filtering_type == 2:
        news_items = [x for x in news_items if x.words_count() <= WORDS_NUM_FILTER2]
        news_items = sorted(news_items, key=lambda x: x.score)
